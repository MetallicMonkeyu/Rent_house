...
import com.app.sys.model.SysFunction;
import com.app.sys.model.SysRoleFunction;
...
public class SysFunctionDao {
	...
	public int add(SysFunction sysFunction) {
		...
		String sql = "insert into t_sys_Function(id,module_id,name,code,url,type,description,display_order) " +
				"values(seq_t_sys_function.nextval,:module_id,:name,:code,:url,:type,:description,:display_order)";
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public HashMap<Integer, String> hashRoleFunctionByRole_id(int role_id) {
		...
		for (SysFunction sysFunction : list) {
			hashMap.put(sysFunction.getId(), sysFunction.getUrl());
		}
		return hashMap;
	}
	...
	public List<SysFunction> getAll() {
		String sql = "select t.*,(select name from t_sys_module where id=module_id) module_name from t_sys_function t where type=1 ";
		List<SysFunction> list = jdbcTemplate.query(sql, new BeanPropertyRowMapper(SysFunction.class));
		return list;
	}
}
